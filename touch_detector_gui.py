...
SELECTED_EPCS = [
    "E2806995000050154D38354E",
    "E2806995000040154D38314E",
    "E2806995000050154D38394E",
]
...
class LiveTagMetrics:
    ...
    def _update_touch_status(
        ...
        touched, reason = self._student_touch_rule(
            rssi_drop=rssi_drop,
            rate_drop=rate_drop,
            no_read=no_read,
            was_touched=self.touch_state[epc],
        )
        self.touch_state[epc] = touched
    ...
    def _student_touch_rule(
        ...
        Available inputs:
        - rssi_drop: baseline RSSI - current RSSI, in dB.
          Example: baseline -45 dBm and current -58 dBm gives 13 dB.
        - rate_drop: fraction of read-rate loss from baseline.
          Example: 0.70 means the tag is being read 70% less often.
        - no_read: True when the tag has not appeared recently.
        - was_touched: True if this tag was marked touched in the previous update.

        Suggested constants to use:
        - RSSI_DROP_TOUCH_DB
        - RSSI_DROP_RELEASE_DB
        - RATE_DROP_TOUCH_FRACTION
        - RATE_DROP_RELEASE_FRACTION

        The following TODO shows a simple OR logic implementation.
        You should implement more robust logic for detecting touch and release.
        """

        # TODO 1: Decide what should happen when no_read is True.
        # Hint: if the tag disappears, it is often a strong touch signal.
        if no_read:
            return True, "no recent reads"

        # TODO 2: Decide what RSSI drop should trigger a touch.
        # Hint: compare rssi_drop with RSSI_DROP_TOUCH_DB.
        # Further consideration: make the threshold tag-specific.

        # TODO 3: Decide what read-rate drop should trigger a touch.
        # Hint: compare rate_drop with RATE_DROP_TOUCH_FRACTION.

        # TODO 4: Add release logic using was_touched.
        # Hint: do not switch back to CLEAR until the signal has mostly recovered.

        # Starter behavior: the GUI runs, but it will not detect touches until
        # students implement the rules above.
        return False, "student rule not implemented"
